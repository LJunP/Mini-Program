...
3. **真实音频流仍缺 8/12**
   * *已完成*：4 首明确授权录音已上传 CloudBase 私有目录，由 `getStudyData` 签发临时地址；详情页展示许可，用户点击后播放，挂载时不自动播放。
   * *实测*：2026-07-25 开发者工具播放《流水》成功，识别总时长 08:13，Console Errors 为 0。
   * *剩余*：8 首尚未找到曲目匹配且录音权利可验证的来源，保持 `audioSrc` 为空，不使用相似替代录音。
