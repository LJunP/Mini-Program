...
| `utils/tracker.js` | ✅ 已完成 | 本地埋点 SDK（支持每 10 秒定时批量落盘） |
...
### 🔴 核心必选功能 (P0 - 上线前必须完成)
1. **真实微信登录与授权**：将 `utils/auth.js` 里的 Mock 逻辑替换为调用微信 `wx.login`，并对接后端接口获取真正的业务 Token 与手机号。
2. **后端 API 对接**：根据 `docs/technical/API接口文档.md` 设计，将 `request.js` 中的 `enableMock` 开关关闭，跑通真实服务端的 HTTP/RESTful 接口逻辑。
3. **云开发数据库与内容同步**：对接云端数据库，实现用户收藏、足迹历史、学习进度的云端持久化存储。
4. **多媒体资源托管**：替换 `assets/` 中占用庞大体积的音频、图片资源至 CDN 存储，减小小程序主包与分包体积。

### 🟡 体验与质量增强 (P1 - 中优先级)
5. **单元测试与集成测试**：针对 `utils/search-engine.js` 及 `utils/persona.js` 等核心逻辑搭建自动化测试框架（如 Jest）。
