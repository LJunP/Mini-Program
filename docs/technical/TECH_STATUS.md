...
4. **TTS 供应商额度已耗尽（上线阻断）**
   * *实测事实*：2026-07-25 从面试题详情页调用已部署 `tts` 云函数，腾讯云返回
     “resource pack allowance has been exhausted”；这说明密钥和接口链路可达，但当前无可用额度。
   * *已处理*：客户端首次失败即停止，不再逐段重复请求；额度恢复后仍需重新完成合成、播放、离页停止三项验证。
