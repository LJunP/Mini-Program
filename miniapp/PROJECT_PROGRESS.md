...
### 🔴 核心必选功能 (P0 - 上线前必须完成)
1. **TTS 额度恢复与复测**：云函数密钥链路已打通，但 2026-07-25 腾讯云返回资源包额度耗尽；恢复额度后必须重测合成、播放、离页停止。
2. **跨账号隐私验收**：投稿已改为默认私密，云函数也只允许本人读取私密内容；仍需用第二个真实账号验证私密详情不可见。
3. **公众平台隐私声明**：按实际上线数据流完成微信公众平台的用户隐私保护指引，提交前逐项核对。
4. **真实音乐流**：产品图片已全部迁移至 CloudBase 私有存储，最终临时预览包 1176 KB；音乐模块仍缺承诺的真实可播放音频。
