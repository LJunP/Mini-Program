...
2. 创建与当前查询一致的 UGC 复合索引，并确认 `_openid + client_id`
   唯一复合索引真实生效。
3. 在微信开发者工具刷新该小程序的发布授权，部署本轮改动的 `login`、`sign`、
   `ugc`；确认 UGC 超时为 10 秒且 `getMyPosts.server_build` 与本地构建一致。
   远端 UGC 当前仍是 timeout=3，CLI 曾返回 41002；腾讯云网页已登录或主控制台
   “上海 0 个环境”都不能替代开发者工具授权与真实部署验证。
